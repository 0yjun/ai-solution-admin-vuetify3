...
export function usePage<T> (fetchUrl: string,defaultSize = 10) {
  const page = ref<Page<T>>(createEmptyPage<T>(defaultSize))
  const errorMessage = ref('')

  async function fetchPage (params: PagingParams): Promise<boolean> {
    errorMessage.value = ''
    try {
      const { data: api } = await axios.get<ApiResponse<Page<T>>>(fetchUrl, { params, paramsSerializer: p =>qs.stringify(p, { arrayFormat: 'repeat' }) })
